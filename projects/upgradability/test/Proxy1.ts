...
describe("Lock", function () {
    ...
    async function deployOneYearLockFixture() {
        ...
        const proxy1 = await Proxy1.deploy(logic1.address, await owner.getAddress());

        return {lock, unlockTime, lockedAmount, owner, otherAccount, logic1, proxy1};
    }

    describe("Proxy", function () {
        it("Proxy has value Logic haven't value", async function () {
            ...
            const readResponse = await ownerAddr.call({
                to: proxy1.address,
                data: readData
            });
    ...
    describe("Deployment", function () {
        it("Should set the right unlockTime", async function () {
            ...
            const readData = await logic1.interface.encodeFunctionData("value");
            const readResponse = await ownerAddr.sendTransaction({
                to: proxy1.address,
                data: readData
            });
            console.log(`proxy get value: ${await readResponse.value}`)
            console.log(`logic get value: ${await logic1.value()}`)
        });
